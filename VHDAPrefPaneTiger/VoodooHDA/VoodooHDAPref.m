...
//		[HDAselector addItemWithTitle:trimIORegistryPathForDisplay(s)];
	...
	{
		NSString *s = [services objectAtIndex:i];
		[HDAselector addItemWithTitle:trimIORegistryPathForDisplay(s)];
	}
...
// attempt to load complete hzize on Tiger   
/*    
 	NSView *view = [self mainView];
	[view setFrame:NSMakeRect(0, 0, 677, 499)];
	[view setAutoresizingMask:
     NSViewWidthSizable | NSViewHeightSizable];
*/
    // TEST LOG
//    NSLog(@"MAINVIEW FRAME: %@", NSStringFromRect([[self mainView] frame]));
//    NSLog(@"WINDOW FRAME: %@", NSStringFromRect([[[self mainView] window] frame]));
 ...
 */
    ...
    for (i = 0; i < [services count]; i++)
    {
        NSString *device = [services objectAtIndex:i];
        ...
        [innerSettings setObject:[NSData dataWithBytes:chInfo
                                                length:(chInfo->numChannels * sizeof *chInfo)]
                          forKey:device];
    }
  ...
  //   NSError *error = nil;
     ...
     NSData *plistData = [NSPropertyListSerialization dataFromPropertyList:outerSettings
     format:NSPropertyListXMLFormat_v1_0
     errorDescription:nil];
     
     if (!plistData)
     {
     NSLog(@"plist serialization failed");
     goto failure;
     }
     
     if (![plistData writeToFile:nPath atomically:YES])
     {
     NSLog(@"write failed to path: %@", nPath);
     goto failure;
     }
